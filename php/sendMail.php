...
if(!empty($name) && !empty($firstName)  && !empty($objet) && !empty($email) && !empty($text)){
	...
	if(strlen($name) < 3 || strlen($name) > 16) {
		echo "Votre nom doit être compris entre 3 et 16 caractères";
		exit();
	}else if(strlen($firstName) < 3 || strlen($firstName) > 16) {
		echo "Votre prénom doit être compris entre 3 et 16 caractères";
		exit();
	}else if(strlen($objet) < 3){
		echo "Votre objet doit être supérieur à 3 caractères";
		exit();
	}else if(is_numeric($name)) {
		echo "Le nom ne peut contenir de chiffre.";
		exit();
	}else if(is_numeric($firstName)) {
		echo "Le prenom ne peut contenir de chiffre.";
		exit();
	}else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
		echo "Ceci n'est pas une adresse mail valide<br />";
		exit();
	}else{
